...
function getpw()
{
    ...
    //SQL prepration
    $stmt = $dbh->prepare('SELECT id,name,sb_name,join_date,money,link FROM gs_sb_table WHERE name = :name AND email=:email');
...
function sum()
{
    ...
    //２．データ抽出SQL作成
    $stmt = $pdo->prepare("SELECT SUM(money) FROM gs_sb_table WHERE name =:name AND email=:email");
    $stmt->bindValue(':name', $name, PDO::PARAM_STR);
    $stmt->bindValue(':email', $email, PDO::PARAM_STR);
    $status = $stmt->execute();
